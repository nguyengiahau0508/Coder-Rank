...
export class CreateComponent {
  ...
  onSummit() {
    this.isLoading.set(true);
    this.contestsService.createContest(this.contest(), this.fileSelected()!).subscribe({
      next: (res) => {
        this.isLoading.set(false);
        this.toastr.success('Contest created successfully!', 'Success');
        this.router.navigate(['/contests/edit', res.data.id]);
      },
      error: (err) => {
        console.error(err);
        this.isLoading.set(false);
        this.toastr.error('Failed to create contest!', 'Error');
      }
    });
  }
